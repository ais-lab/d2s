...
class Base_Collection():
    ...
    def do_augmentation(self, image, image_infor_class, debug = False):
        ...
        if self.cfg.train.augmentation.dsacstar.apply:
            ...
            transf_image, points2Ds, camera, pose = aug.dsacstar_augmentation(
                transf_image, self.cfg.train.augmentation.dsacstar, points2Ds, camera, pose)
        ...
        if self.cfg.train.augmentation.dsacstar.apply:
            image_infor_class = aug.correct_points_inside_image(transf_image.shape, image_infor_class)
        if debug:
    ...
    def image_loader(self, image_name, augmentation=False, debug = False):
        ...
        scale = 1.0
        if resize_force and (max(ori_size) > resize_max):
    ...
    def detect_points2D(self, image_name):
        ...
        # scale = torch.from_numpy(data['scale']).float().to(self.device)
        scale = data['scale']
        if resize_force and (scale != 1.0):
