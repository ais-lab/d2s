...
def open3d_vis_3d_lines(lines3D, cameras =None, poses=None, width=2, ranges=None, scale=1.0):
    """
    ...
    if poses is not None:
        for pose, camera in zip(poses, cameras):
            add_camera(vis, pose, camera, scale=0.1, gt = True)
    vis.run()
    vis.destroy_window()
